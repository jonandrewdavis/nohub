...
import type { SessionData, SessionId, SessionSocket } from "@src/sessions/session";
import type { SessionRepository } from "@src/sessions/session.repository";

export class BroadcastService {
  constructor (private reactor: () => NohubReactor, private sessionRepository: SessionRepository) {}
  ...
  broadcast(lobby: Lobby, command: CommandSpec): Map<SessionId, Exchange<SessionSocket>> {
    const result = new Map();

    for (const sessionId of lobby.participants) {
      const session = this.sessionRepository.find(sessionId);
      if (!session)
        continue; // Shouldn't happen, unless lobby participants are not cleared up on client disconnect
